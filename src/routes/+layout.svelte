<svelte:head>
    <title>Infoportal.app | Home</title>
    <meta name="description" content="Infoportal.app place to publish your questions, answers, thoughts and opinions. Blochain based." />
</svelte:head>

<script lang="ts">
	import Header from "$lib/components/Header.svelte";
	import Footer from "$lib/components/Footer.svelte";
	import Loader from '$lib/components/Loader.svelte';
	import { ProviderCommons } from "$lib/classes/Provider";
	import { goto } from "$app/navigation";
	import { onMount } from "svelte";

	// IMPORT GLOBAL STYLE
	import '../styles/main.css';

	let showLoader: boolean = true;

	onMount(async () => {
        // begin checking for Metamask presence
        const walletFound = await ProviderCommons.startMetamaskCheck();
        if (walletFound) {
			console.log(`<MainPage> Provider found. Redirecting to connect page.`);
			goto('/connect');
		} else {
            console.log(`<MainPage> Wallet not found. Redirecting to instructions page.`);
            goto('/instructions');
        }
		showLoader = false;
    });
</script>

<main>
	<Header />

	<!-- CHECK IF WE SHOULD SHOW DETECTION LOADER -->
	{#if showLoader}
		<page-center>
			<Loader showDetector />
		</page-center>
	{:else}
		<main-content>
			<slot />
		</main-content>
	{/if}
</main>
<Footer />

<style>
    main {
		display: flex;
		flex-direction: column;
		justify-content: stretch;

		/* position: absolute; */
		width: 100vw; 
		height: 100vh;
		/* background-image: linear-gradient(to top, #a3bded 0%, #6991c7 100%); */
		background: #667db6;  /* fallback for old browsers */
		background: -webkit-linear-gradient(to top, #667db6, #0082c8, #0082c8, #667db6);  /* Chrome 10-25, Safari 5.1-6 */
		background: linear-gradient(to top, #667db6, #0082c8, #0082c8, #667db6); /* W3C, IE 10+/ Edge, Firefox 16+, Chrome 26+, Opera 12+, Safari 7+ */
	}

	main-content {
		flex-grow: 1;
		overflow-y: auto;
		display: flex;
    }

	page-center {
		flex-grow: 1;
		display: flex;
		justify-content: center;
		align-items: center;
		/* background-color: #64b3f4; */
	}


	@media (min-width: 640px) {
		main {
			/* background-image: linear-gradient(60deg, #64b3f4 0%, #c2e59c 100%); */
			background-image: linear-gradient(60deg, #4ca3eb 0%, #dce9ce 100%);
			max-width: none;
			max-height: none;
		}
	}

	@media (min-width: 280px) {
		main {
			max-width: none;
			max-height: none;
		}
	}
</style>
